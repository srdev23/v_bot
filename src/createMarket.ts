...
export async function createMarket() {
  try{
    console.log(chalk.blue("\n> Starting new market creation process...\n"));
    ...
    // if(solBal < 0.3) {
    //   console.log(chalk.red(`Insufficient SOL balance: ${solBal}/0.3 SOL`));
    //   process.exit(1);
    // }

    const BaseToken = new Token(TOKEN_PROGRAM_ID, myToken, tokenInfo.decimals);
    ...
    for (const iTx of willSendTx) {
      if (iTx instanceof VersionedTransaction) {
        iTx.sign([mainWallet]);
        console.log('kk', iTx.serialize().length);
    ...
    const market_id = createMarketInstruments.address.marketId.toString();
    FileService.saveMarketId(market_id);
    console.log(chalk.green(`Created new market id ${market_id}: https://solscan.io/account/${market_id}`));
  }catch(e){
